...
const FLOW = [
  { icon: Camera, n: '01', title: '拍下公文', detail: '罚单、Show Cause、押金通知' },
  { icon: Sparkles, n: '02', title: 'Gemini 读懂', detail: '多模态识别事实、金额与期限' },
  { icon: Search, n: '03', title: '核验依据', detail: 'Google Search Grounding 检索官方来源' },
  { icon: ListChecks, n: '04', title: '生成行动', detail: '风险、权利、材料与下一步' },
  { icon: Mail, n: '05', title: '发起申诉', detail: '英文草稿保存到 Gmail' },
];
...
export default function JudgingProof() {
  return (
    <section className="mb-8 space-y-3" aria-label="Serene 核心能力与验证证据">
      <div className="overflow-hidden rounded-3xl border border-hairline bg-surface-card shadow-sm">
        <div className="flex flex-col gap-4 border-b border-hairline px-5 py-5 md:flex-row md:items-end md:justify-between md:px-6">
          <div>
            ...
            <h3 className="text-xl font-bold text-ink md:text-2xl">一封看不懂的信，五步变成可执行的下一步</h3>
          </div>
          <p className="max-w-sm text-xs leading-5 text-muted">路演只演示这一条核心链路；生活圈与社区能力作为后续扩展，不参与主叙事。</p>
        </div>
        ...
        <div className="grid border-t border-hairline bg-surface-soft/55 lg:grid-cols-[1fr_auto_1fr_auto_1fr] lg:items-center">
          <div className="px-5 py-4">
            ...
            <p className="mt-1 text-xs font-semibold text-ink">图片 + 签证 / 学校 / 州别上下文</p>
          </div>
          ...
          <div className="px-5 py-4">
            ...
            <p className="mt-1 flex items-center gap-1.5 text-xs font-semibold text-ink"><Database size={13}/> Firebase 案头 + Gmail 草稿</p>
          </div>
        </div>
      </div>
      ...
      <details className="group overflow-hidden rounded-2xl border border-hairline bg-surface-card">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-ink">5 个模拟澳洲新居民情境测试</p>
              <p className="mt-0.5 text-[11px] text-muted">覆盖留学生、新移民、Working Holiday、陪读家庭与外派人员 · 可复现，不冒充访谈</p>
            </div>
          </div>
        ...
        <div className="border-t border-hairline px-5 py-5">
          ...
          <div className="grid gap-3 md:grid-cols-5">
            {PERSONAS.map((p) => (
              <article key={p.name} className="rounded-2xl border border-hairline bg-surface-soft/55 p-3.5">
                ...
                <p className="mt-0.5 text-[10px] text-muted">{p.context}</p>
                <p className="mt-3 text-[10px] font-bold text-primary">测试：{p.case}</p>
                <p className="mt-1 text-[10px] leading-4 text-body">通过标准：{p.expected}</p>
              </article>
            ))}
          </div>
          <p className="mt-4 flex items-center gap-1.5 text-[10px] text-muted"><ExternalLink size={11}/> 所有测试均可用页面内置案例和语言切换器现场复现。</p>
        </div>
      </details>
    </section>
  );
